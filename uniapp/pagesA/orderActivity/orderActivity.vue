		...
		<view class="background-box">
			...
			<view class="order-goods" v-for="(item,index) in coententlist" :key="index">
				...
				<view @click="particulars(item.id)">
					<text class="order-goods-title">{{item.venue_name}}</text>
					...
					<view class="order-goods-content">
						<image :src="item.venue_img" mode=""></image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			particulars(id) {
				uni.navigateTo({
					url: '/pagesC/sufActivitiesTwo/sufActivitiesTwo?id=' + id + '&flag=' + this
						.ismerchant
				})
			},
			pay(oder_on, price) {
...
<style scoped lang="scss">
	...
	.order-goods-content {
		...
		.order-goods-content_title {
			...
			.order-goods-content_title_bottom {
				margin-top: 20rpx;
			}
		}
	}
